...
export const firstReducer = (state = initialState, action) => {
  switch (action.type) {
    case "FIRST-ADD-POST":
      let mess = {
        idd: uniqid(),
        name: state.newMessageBody1,
        recepie: state.newMessageBody2,
        src: "6.jpg",
      };
      return {
        ...state,
        first: [...state.first, mess],
        newMessageBody2: "",
        newMessageBody1: "",
      };
    ...
    case "FIRST-UPDATE-NEW-POST-CHANGE":
      return {
        ...state,
        newMessageBody2: action.recepie,
      };

    case "FIRST-UPDATE-INPUT-CHANGE":
      return {
        ...state,
        newMessageBody1: action.name,
      };

    case "FIRST-REMOVE":
      let newArrFirst = state.first.filter((item) => item.idd !== action.idd);
      return {
        ...state,
        first: [...newArrFirst],
      };
...
export const updateFirstInputActionCreator = (title) => ({
  type: "FIRST-UPDATE-INPUT-CHANGE",
  name: title,
});

export const updateFirstActionCreator = (text) => ({
  type: "FIRST-UPDATE-NEW-POST-CHANGE",
  recepie: text,
});
